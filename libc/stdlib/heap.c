...
#if defined (__is_libk)
void print_heap_header_data() { // NOTE: DEBUG FUNCTION
	header_t *current_block = first_block;
	while (current_block) {
...
extern void print_memory(void *, size_t);

void print_heap_memory_data() { // NOTE: ALSO A DEBUG FUNCTION
	header_t *current_block = first_block;
	while (current_block) {
		printf("Block %p size %zu is_free %d\n", current_block, current_block->size, current_block->is_free);
		print_memory((void *) current_block, sizeof(header_t) + (size_t) current_block->size);
		current_block = current_block->next;
	}
	return;
}
...
// heap.c - Oh wow, this is quite a bit, isn't it? This is heap.c, which contains a set of closely related functions that need
// to work together to maintain the "allocated heap" form of data. If you notice, there's a lot of pointer structs in here,
// which is why this place has the kindly-given nickname of POINTER HELL.